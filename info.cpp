...
int versionOp(set<string>&, vector<string> &arguments) {
...
int helpOp(set<string> &options, vector<string> &arguments) {
	...
	cerr << "Minecraft SERver MANager - Utility for easy server managements"
		...
		"    interactive                        Same as --interactive or -i\n"
		...
		"    [-msg] verify                      Verify data in config\n"
		...
		"    -i    --interactive                Start interactive console\n"
		...
		//            "    -c    --clear-gone                 Clear 'GONE' entries\n"
	...
	return arguments.empty() && !options.contains("functional-help") ? EXIT_FAILURE : 0x0;
}
