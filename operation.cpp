...
int backupOp(set<string> &options, vector<string> &arguments) {
    ...
    if (!(deep && action == "load") && !root.root("SERVERS")(arguments[2])) printError("Server with name \"" + arguments[2] + "\" doesn't exist");
    ...
    path_node &pathArchive = ensureExists(root.paths["BACKUP"][deep ? "archive" : "server"])[arguments[2] + ".bak"];
    path_node &dirServer = root.paths["GENERAL"]["server"][arguments[2]];
    ...
    if (action == "save") {
        ...
        if (deep) {
            ...
            string_node config;
            config["SERVER"] = root.root["SERVERS"][arguments[2]];
            config.children.erase("STATUS");
            config.children.erase("META-CONFIG");
            //TODO store "deep" special data
        }
